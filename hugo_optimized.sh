...
main() {
  ...
  if hash exiftool 2>/dev/null; then
    find static -type f -iregex ".*\.\(jpeg\|jpg\|png\)" -print0 | xargs -0 -n1 -P$NUM_PARALLEL_JOBS bash -c 'process_exif "$0"'
  else
  ...
  if hash jpegoptim 2>/dev/null; then
    find static -type f -iregex ".*\.\(jpeg\|jpg\)" -print0 | xargs -0 -n1 -P$NUM_PARALLEL_JOBS bash -c 'optimize_jpeg "$0"'
  else
  ...
  if hash optipng 2>/dev/null && hash pngcrush 2>/dev/null; then
    find static -type f -iregex ".*\.\(png\)" -print0 | xargs -0 -n1 -P$NUM_PARALLEL_JOBS bash -c 'optimize_png "$0"'
  else
  ...
  if hash gifsicle 2>/dev/null; then
    find static -type f -iregex ".*\.\(gif\)" -print0 | xargs -0 -n1 -P$NUM_PARALLEL_JOBS bash -c 'optimize_gif "$0"'
  else
  ...
  if hash cwebp 2>/dev/null; then
    find static -type f -iregex ".*\.\(webp\)" -print0 | xargs -0 -n1 -P$NUM_PARALLEL_JOBS bash -c 'optimize_webp "$0"'
  else
